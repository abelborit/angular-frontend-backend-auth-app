...
- Vamos a aprovisionar nuestra base de datos MongoDB en Railway pero recordar que hay varios servicios gratuitos hasta cierto punto que nos permiten aprovisionar la base de datos MongoDB como MongoAltas pero en este caso lo haremos con Railway que también es gratuito hasta cierto punto.
  ...
  - Railway:
    ...
    - Ahora que ya está desplegado el backend entonces faltaría probarlo. Podemos ir a la pestaña settings y si bajamos un poco veremos que dice Domains y ahí se puede exponer el servicio a internet, donde por defecto no está habilitado (imagen 26) porque puede ser que queramos usar el backend para que esté conectado a una red de otras aplicaciones que podamos tener ahí. Técnicamente podemos hacer como una red interna protegida del mundo exterior y solo exponer ciertas aplicaciones. En este caso como queremos exponerla al mundo exterior entonces presionamos Generate Domain para generar un dominio y nos dará un nombre que podemos cambiar (imagen 27 - imagen 28) el cual ese nombre es el que tendríamos que cambiar en la aplicación de frontend que use el dominio antiguo. Si abrimos o vamos a esa dirección nos aparecerá algo similar a "not found" (imagen 29) pero esto está bien ya que nosotros no estamos sirviendo nada en la raiz pública. Luego vamos a postman y probamos esa nueva URL pero darse cuenta que tenemos que colocar https:// al principio porque ya estamos trabajando con un backend de producción (imagen 30)
    - Con lo anterior ya se tiene la base de datos y el backend desplegado, ahora faltaría desplegar el frontend. Lo que se puede hacer es que en la misma aplicación de nest colocar la aplicación de angular en la carpeta pública para que la gente la pueda consumir y cuando se entre al url del backend se pueda ver la aplicación de angular pero en este caso lo haremos mediante Netlify. Como ya tenemos la url que usaremos para el backend entonces hay que modificar las variables de entorno del frontend para usar esa nueva url generada (imagen 31) y luego hacer el build de producción del frontend. Entramos a Netlify y crearemos un nuevo proyeceto y haremos el deploy manualmente y dejaremos caer la carpeta dist/nombre_de_la_app para que se pueda desplegar.
